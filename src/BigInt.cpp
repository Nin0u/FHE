...
BigInt powmod(const BigInt& base, int exp, const BigInt &mod) {
    if (exp < 0) {
        if (base == 0)
            throw std::logic_error("Cannot divide by zero");
        return abs(base) == 1 ? base : 0;
    ...
    while (exp > 1) {
        if (exp % 2)
            result_odd = (result_odd * result) % mod;
        result = (result * result) % mod;
        exp /= 2;
    }

    return (result * result_odd) % mod;
}
...
std::tuple<BigInt, BigInt, BigInt> xgcd(const BigInt &num1, const BigInt &num2) {
    BigInt cpy1(num1), cpy2(num2);
    BigInt prevx = 1, x = 0, prevy = 0, y = 1;
    ...
    return std::tie(cpy1, prevx, prevy);
}
