...
UnboundedInt::UnboundedInt(std::string s){
	...
	for(unsigned int i = 0; i < s.size(); i++){
		...
		for(unsigned int  j = s.size() - i; j > 1; j--){
			digit.multiply(ten);
		}
		add(digit, false);
	}
...
void UnboundedInt::rotate(const UnboundedInt& B, unsigned int size){
	...
	for(unsigned int i = 0; i < B.blockSize(); i++){
		for(unsigned int j = 0; j < UINT_SIZE; j++){
...
void UnboundedInt::shiftBlockL(unsigned int shift, bool cutoff){	
	...
	for(unsigned int i = (A.size() - 1); i >= shift; i--){
		A[i] = A[i - shift];
	}
...
void UnboundedInt::shiftBlockR(unsigned int shift){	
	...
	for(unsigned int i = 0; i < (A.size() - shift); i++){
		A[i] = A[i + shift];
	}
...
void UnboundedInt::add(const UnboundedInt &B, unsigned int &carry, bool neg){	
	...
	unsigned int prev;
	for(unsigned int i = size; i < A.size(); i++){
		prev = A[i];
		A[i] = A[i] + carry;
...
void UnboundedInt::divide(const UnboundedInt &B, UnboundedInt &Rem, unsigned int divSize){
	...
	for(unsigned int i = 0; i < mult.blockSize(); i++){
		if(mult.blockAt(i) == 0){
			empty++;
		}
		else{
	...
	for(unsigned int k = 0; k < base.blockSize(); k++){
...
std::ostream& operator<<(std::ostream& os, const UnboundedInt &B){
	...
	if(B.blockSize() == 1 && B.blockAt(0) == 0){
		...
		for(unsigned int i = ((B.blockSize() << 5) -1 ); i >= 0; i--){
			if(output.at(i) != 0){
				leadingZero = false;
			}
...
std::string UnboundedInt::to_binary(){
	std::stringstream ss;
	for(unsigned int i = A.size() - 1; i >= 0; i--){
		for(unsigned int j = (UINT_SIZE - 1); j >= 0; j--){
			ss << getBit(A[i],j);
		}
	}	
	return ss.str();
